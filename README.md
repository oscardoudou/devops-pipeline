...
In addition, while implemeting "got" module, the tests continuously failed because our implemented node version didn't support it, because of which we had to fall back to the "request" module. It worked brilliantly when combined with the pm2 shell commands using the "shelljs" module. Apart from this, the existing ```package.json``` of the repository required the test command as "mocha" for ```npm test``` to invoke the test script via Mocha. 
